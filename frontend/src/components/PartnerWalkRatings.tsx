...
import { useWalkRatingsByProfile } from '@/hooks/useQueries';
import type { WalkRating } from '../backend';

interface PartnerWalkRatingsProps {
  profileId: string;
}

export default function PartnerWalkRatings({ profileId }: PartnerWalkRatingsProps) {
  const { data: ratings, isLoading, error } = useWalkRatingsByProfile(profileId);
    ...
    try {
      ...
      ratings.forEach((rating) => {
        ...
        try {
          ...
          console.error('[PartnerWalkRatings] Error processing rating date:', err);
        }
      });

      return groups;
    } catch (err) {
      console.error('[PartnerWalkRatings] Error grouping ratings:', err);
      return {};
    }
  };
  ...
  if (error) {
    console.error('[PartnerWalkRatings] Error loading ratings:', error);
    return (
      <Card className="border-primary/20">
        <CardHeader>
          <CardTitle className="text-2xl">Walk Ratings</CardTitle>
      ...
      <Card className="border-primary/20">
        ...
        <CardContent>
          <p className="text-muted-foreground text-center py-8">
            No walk ratings yet. Complete a walk to add the first rating! 💪
          </p>
        </CardContent>
      </Card>
    );
